...
import { postCGSData, getCGSData } from "../../../Services/employeeServices";
...
const CGS = ({ id }) => {
  ...
  const [CGSData, setCGSData] = React.useState(null);
  ...
  const handleSave = () => {
    ...
    postCGSData(data, setRefreshVCare, refreshVCare, setDataSaveLoading);
  };

  React.useEffect(() => {
    getCGSData(id, setCGSData, setLoading);
  }, [id, refreshVCare]);
