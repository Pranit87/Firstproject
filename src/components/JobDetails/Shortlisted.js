...
import { getShortlisted, getResumeDetails } from "../../Services/jobsService";
...
function Shortlisted(props) {
  ...
  const [smsInfo, setSmsInfo] = React.useState({ from: "", to: "" });
  ...
  React.useEffect(() => {
    if (jobId || props.reloadInterview) {
      getShortlisted(jobId, page, setShortlisted, setTotalPage, setLoading);
    }
                        ...
                        <td className="py-2">
                          {row.sourced_date &&
                            convertDateStringToLocalDatetime(
                              row.sourced_date.split(" ")[0].replace(`"`, "")
                            ).toLocaleDateString()}
                        </td>
                        <td className="py-2">
                          {" "}
